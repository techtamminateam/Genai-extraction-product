...
def prompt_template_commercial_flood(large_text=None):
    ...
    11. Location
        ...
        - Do not extract lCo Insuranceocation data from other sections, schedules, or tables.
        - need to extract loc label and its value along with address
        - Example:"Loc 1  1731 W Business U.S. 60, ---- if any one those are lisited in the document we have to extract in location                                                                                                   
        - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60  
        - if one or more location is present we have to extract like this ---
...
def prompt_personal_boat(large_text=None):
    return f"""
    You are an expert insurance data extractor.
        The following text is extracted from a Personal Boat insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        9. Paid in Full Discount
        10. Miscellaneous Premium (the miscellaneouspremium is mentioned as miscellaneouspremium only there is no other alternatives names)
            - Only if labeled exactly 'miscellaneous premium'.
        11. Location Address
        - no need to extract Property Location 
        - Extract location details **only from the "declaration of location" page or section**.
        - Do not extract location data from other sections, schedules, or tables.
        ...
        - Example:"Loc 1  1731 W Business U.S. 60, ---- if any one those are lisited in the document we have to extract in location                                                                                                   
        - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60  
        12. Limits of liabilty
            - Extract liability limits with lables and values
        13. Property Damage
            -Extract property damage coverage amount with full details
        14. Uninsured or Underinsured Boater 
            -Extract coverage for uninsured/underinsured boater with full details
        15. Unisured Boater Property Damage
            - Extract property damage by unisured boater
        16. Medical Payments
            - Extract Medical payment Coverage
        17. Comprehensive
            - Extract Comprehensive Coverage Value
        18. Collision
            - Extract Collision Coverage Value
        19. Coastal Navigation
            - Extract coastal navigation coverage
        20. Baot or Watercraft Info
            - Extract details of Boat/Watercraft
        21. Driver Info
            - Extract driver info with full details
        22. Deductible
            -extract the with labels Deductible values.
        23. Terrorism
            - From premium section or separate terrorism page.
        24. Exclusions
            - Exclusion listed in the policy
        25. Additional Coverage
            - Extract all the limits with labels and which not include in main datapoints additional coverage items.
        26. Additional Interest
            - Extract additional interest details only
        27. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
            -Format EXACTLY as:
...
def prompt_liqour_liabilty(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Liqour Liability insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60  
        12. Business Classification 
            - Extract the insured's business classification exactly as written.
        13. Limits of Liability
            - Extract all specified limits of liability from the declarations or limits section.
        14. Deductible or Retention
            - Extract deductible or retention values from the appropriate policy section.
        15. Retro Date
            - Extract the retroactive date when listed.
        16. Prior and Pending Date
            - Extract the prior and pending litigation date when provided.
        17. Continuity Date
            - Extract the continuity date from declarations or continuity endorsement.
        18. Underlying Insurance
            - Extract underlying policy details including carrier, limits, and policy numbers.
        19. Terrorism
            - Extract terrorism coverage details whether located in the premium section or on a separate terrorism page.
        20. Exclusions
            - Extract all exclusions exactly as listed under the exclusions section.
        21. Additional Coverage
            - Extract additional coverage other than the primary coverages.
            - Need to extract values with their labels
        22. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
...
def prompt_surety_bond(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Surety Bond insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        ### Datapoints to Extract ###
        1. Bond Number
            - Extract only if field explicitly labeled  Bond Number
        2. Bond Description
            - Extract only if field explicitly labeled  Bond Description
        3. Bond Amount
            - Extract only if field explicitly labeled  Bond Amount
        4. Premium
            - Extract all the data present in the all premiums with labels
        5. Term
            - extract EFFECTIVE DATE  and EXPIRATION DATE under this term data point 
        6. Principal Address
            - Extract only if field explicitly labeled  Principal Address
        7. Obligee Name
            - Extract only if field explicitly labeled  Obligee Name
        8. Obligee Address
            - Extract only if field explicitly labeled  Obligee Address
        9. Indemnitor Name
            - Extract only if field explicitly labeled  Indemnitor Name
        10. Indemnitor Address
            - Extract only if field explicitly labeled  Indemnitor Address
        11. License Number
            - Extract only if field explicitly labeled  License Number
...
def prompt_professional_liability(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Professional Liabilityinsurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        ### Datapoints to Extract ###
        1. Name Insured
            - Extract all primary names which are mentioned to a single primary name label 
        2. Other Named Insured
            - sometimes it given as DBA and in some cases it is present below the name insured and do not extract from additional named insured section 
            - Name Insured datapoint name should not be extracted in this
            - No need extract "DBA" in value
        3. Additional Insured (it will be with same name or other named insured or DBA ) --- for this data point ,  it has a sapreate page mentioned as Name insured schedule page or Additional insured 
        4. Mailing Address 
        5. Policy Number
        ...
        - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60
        12. Limits of Liability
            - Extract all specified limits of liability from the declarations or limits  with labels and values.
        13. Deductible or Retention
            - Extract deductible or retention values from the appropriate policy section.
        14. Retro Date
            - Extract the retroactive date when listed.
        15. Prior and Pending Date
            - Extract the prior and pending litigation date when provided.
        16. Continuity Date
            - Extract only if field explicitly labeled Continuity Date  with labels
        17. Underlying Insurance
            - Extract data only if it is explicitly labeled as “Underlying Insurance” or clearly listed under an “Underlying Insurance” section.
        18. Terrorism
            - Extract data only if the field, heading, or label explicitly refers to “Terrorism".
            - Do NOT extract information that is not explicitly tied to Terrorism.
            - Extract only when it has a value or limit, do not extract the lines that are tied to "Terrorism"
        19. Exclusions
            - Pass this datapoint as null.
        20. Additional Coverage
            - Extract additional coverage other than the primary coverages.
            - Extract only coverages which have limits.
            - Need to extract values with their labels
        21. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            ...
            -Extract a Form Title ONLY if it appears on the SAME LINE and IMMEDIATELY to the right of the Form Number and Date.
            -If no title appears beside the form number and date on the same line, DO NOT extract a title.
            -Extract only if FormNumber exists
            -Do not extract titles from any other position (different line, different column, separated by layout, etc.).
            ...
            --Return Requirements
                ...
                -FormNumber | MM/YY
        22. Endorsements
            -Extract data only from the Endorsement Declaration page labeled “Endorsements.”
            -From Description of Changes, extract only: Changed, Added, Deleted, Estimated Total Premium, and Paid in Full Discount.
            -Ignore all other pages, sections, labels, duplicates, and do not infer or calculate any values.
        ### Output Rules
        - Return **only** the structured JSON object with the keys above in the exact order.
        - Dates → MM/DD/YYYY
        - Currency → keep "$" and commas as shown.
...
def prompt_pollution_liability(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Pollution Liability insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        7. Issuing Company 
        8. Premium
            - Need to extract all total premiums under the Premium declaration section
            - Extract with combination of labels and values
        9. Paid in Full Discount
        10. Miscellaneous Premium (the miscellaneouspremium is mentioned as miscellaneouspremium only there is no other alternatives names)
            - Only if labeled exactly 'miscellaneous premium'.
        ...
        13. Deductible or Retention
            - Extract deductible or retention values from the appropriate policy section.
        14. Rating Info
            - Extract following datapoints under thhis section
            1. Class Code
            2. Classification
            3. Premium Basis
            4. Exposure
            ...
            6. Premium
        15. Retroactive Date
            - Extract only if field explicitly labeled Retroactive Date or Retroactive  with labels
        16. Prior and Pending Date
            - Extract only if field explicitly labeled Prior and Pending Date  with labels
        17. Continuity Date
            - Extract only if field explicitly labeled Continuity Date  with labels
        18. Underlying Insurance
            - extract the underlying insurance  limits  with labels and values.
        19. Terrorism
            - Extract all the data  if field as Terrorsim   (if same label is present just ignore the label , if different label is present then have to extract with label)
        20. Exclusions
            - extract all the data  if field as Exclusions
        21. Additional Coverage
            - extract all the limits with labels and values which are not included in main datapoints anther named as coverages/ limit of insurance /sublimits/additional coverages
        22. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
...
def prompt_personal_liability(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Pollution Liability insurance PDF using a mix of
        digital text, OCR text, and tables.  
        ...
        11. Location
            ...
            - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60
        12. Liability Limits
            - Extract all the liability limits with labels and values.
        13. Medical Payments Limit
            - Extract only if field Medical Payments  (if same label is present just ignore the label , if different label is present then have to extract with label)
        14. Deductible
            - Extract only if field Deductibles or Retension (if same label is present just ignore the label , if different label is present then have to extract with label)
        15. Hazards Rating info
            - Extract following datapoints under this
            1. Class Code
            2. Classification
            3. Premium Basis
            4. Exposure
        ...
        17. Additional Interest
            - extract if field as Additional Interest 
        18. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
            -Format EXACTLY as:
...
def prompt_farm_liability(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Pollution Liability insurance PDF using a mix of
        digital text, OCR text, and tables.  
        ...
        11. Location
            ...
            - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60
        12. The datapoints mentioned from 13 to 22 should follows below description and each datapoint should treated as individual primary key
            - Extract the value **only from the "Farm Liability declaration page or section"**. Do not extract from other sections, schedules, or tables execpt.
            - For each, include both the value (e.g., "$5,000") and any descriptive text that appears after the value (e.g., "Per Occurrence", "Any One Premise", etc.).
            - Output format for each: "<value> <descriptive text after value>" , not like "Medical expenses": "$10,000 Medical Expenses Limit",
            - Example: 
                - "Medical expenses": "$5,000 Per Occurrence"
        ...
        14. Products or Completed Operations Aggregate (Extract  only if field Products or Completed Operations Aggregate)
        15. Personal And Advt Injury (Extract  only if field Personal And Advt Injury)
        16. Medical expenses (Extract if field Medical expenses with labels)
        17. Damage to rented premises (Extract  if field Damage to rented premises)
        18. Each Occurrence (Extract  if field Each Occurrence)
        19. Deductible (extract if field as  deductible with labels if label not present then limit is enough  from General Liability)
        20. Hired Or Non Owned Auto ("extract if field as hired and non owned auto with labels from General Liability)
        21. "Schedule of Hazards Rating Info". Return as an array of dictionaries, also extract LOC no. if it is present:
                -Class Code
                -Classification
                -Premium Basis
                -Exposure
                ...
                -Premium
        22. Additional interest
            - extract if field as Additional Interest from Farm Liability
        23. The datapoints mentioned from 24 to 36 should follows below description and each datapoint should treated as individual primary key
        - (Need to extract the datapoints in individual dictionary but not under the general liability dictionary, And this data should be extracted which is present under the Farm property declaration page only)
        24. Building Value
        25. Business Personal Property Limit - (need to extract prem.no,bldg.no labels and its values along with limit data)
        26. Business Income Limit - (must and should extract complete value of it and if value consists "INCLUDED" then "included" comes in last index of value)
        27. Improvements and Betterments
        28. Wind And Hail
        29. Property Deductible - (need to extract prem.no,bldg.no labels and its values along with limit data and all deductible under this section only)
        30. Co Insurance
            - most of the cases the data present , in where the bulding value is present , otherwsie it is present in the limits page 
            - If there are more than one location or  builidng values are present then along with lacation and bulding value has to bee extract along with lables like mention below
        31. Valuation
            - most of the cases the data present , in where the bulding value is present , otherwsie it is present in the limits page 
            - If more than one bulding values are present, then extract all of them as individual along with their labels
        32. Is Equipment Breakdown Listed
        33. Building Ordinance Or Law Listed Cov A
        34. Building Ordinance Demolition Cost Cov B
        35. Building Ordinance Inc Cost Of Construction Cov C
        36. Additional Interests (extract only from property declaration page only)
        37. Farm Equipment
            - extract  as with labels and LOC and BLDG no if field as Farm Equipment FROM Farm Property LOB 
        38. Covered Vehicles
            - extract  as with labels and LOC and BLDG no if field  Covered Vehicles FROM Farm Property LOB 
        39. Scheduled Personal Property
            - extract  as with labels and LOC and BLDG no if field  Scheduled Personal Property FROM Farm Property LOB 
        40. Terrorism
            - extract all the data  if field as Terrorsim FROM Farm Property LOB   
        41. Exclusions
            - Pass this datapoint as null.
        42. Additional Coverage
            - extract all the limits with labels and values which are not included in main datapoints anther named as coverages/ limit of insurance /sublimits/additional coverages
            - Extrat labels whenever limits present
        43. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
            -Format EXACTLY as:
...
def prompt_garage_liability(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Garage Liability insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        11. Location
            ...
            - Example:"Loc 1  1731 W Business U.S. 60, ---- if any one those are lisited in the document we have to extract in location                                                                                                   
            - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60
        12. Garage Liability Limits
            - Extract the garage limits of the with labels and loc and bldg no 
        13. Division
            - Extract Division if present
        14. Dealers Blanket Coverage
            - Extract Dealers Blanket Coverage  if present
        15. Deductibles
            - extract if field as deductible with labels if label not present then limit is enough
        16. Garagekeepers Coverage
            - Extract the Garagekeepers Coverage  of the with labels and loc and bldg no 
        17. Scheduled Drivers
            - Extract the Scheduled Drivers  of the with labels and loc and bldg
        18. Terrorism
            - Extract data only if the field, heading, or label explicitly refers to “Terrorism".
            - Do NOT extract information that is not explicitly tied to Terrorism.
            - Extract only when it has a value or limit, do not extract the lines that are tied to "Terrorism"
        ...
        20. Additional Interest
            - extract if field as Additional Interest
        21. Additional Coverage
            - extract all the limits with labels and fire which are not included in main datapoints anther named as coverages/ limit of insurance /sublimits/additional coverages
        22. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
...
def prompt_homeowners(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Home Owners insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        4. Mailing Address 
            - it will be starts with number , PO BOX ,text  below named or other named insured 
        5. Physical Address
            - it may have the physical address or area 
        6. Policy Number
        7. Policy Period
        8. Issuing Company 
        9. Premium
            ...
            - Extract with combination of labels and values
        10. Paid in Full Discount
        11. Miscellaneous Premium (the miscellaneouspremium is mentioned as miscellaneouspremium only there is no other alternatives names)
            - Only if labeled exactly 'miscellaneous premium'.
        12. Dwelling Limit
            - it has the naming of dwelling or dwelling limit  extract with labels and loc and bldg no with limits.
        13. Other Structures
            -it has the naming of Other Structures  extract with labels and loc and bldg no with limits.
        14. Fair Rental Value
            -it has the naming of  Fair Rental value  extract with labels and loc and bldg no with limits.
        15. Loss of use
            - it has the naming of Loss of use extract with labels and loc and bldg no with limits.
        16. Personal Property
            - it has the naming of Personal Property  extract with labels and loc and bldg no with limits.
        17. Personal Liability
            - it has the naming of Personal Liability extract with labels and loc and bldg no with limits.
        18. Additional Living Expense
            - it has the naming of Additional Living Expense extract with labels and loc and bldg no with limits.
        19. Medical Payments
            - it has the naming of Medical Payments extract with labels and loc and bldg no with limits.
        20. Deductibles
            - it has the naming of Deductibles extract with labels and loc and bldg no with limits.
        21. Windhail
            - it has the naming of Windhail  extract with labels and loc and bldg no with limits.
        22. Water Backup
            - it has the naming of Water Backup or Backup  extract with labels and loc and bldg no with limits.
        23. from 24 to 30 extract datapoints form "Property Info" declaration section
        24. Roof Age
        25. Year Built
        26. Year Purchased
        27. Square footage
        ...
        30. Basement
        31. Terrorism
            - Extract all the data  if field as Terrorsim
        32. Exclusions
            - extract all the data  if field as Exclusions
        33. Additional Interest
            - extract if field as Additional Interest
        34. Additional Coverage
            - extract all the limits with labels and fire which are not included in main datapoints anther named as coverages/ limit of insurance /sublimits/additional coverages
        35. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
            -Format EXACTLY as:
...
def prompt_personal_auto(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Personal Auto insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        11. Liability limit
            - Extract all the liability limits with labels and values.
        12. Property Damage
            - Extract all the Property Damage with labels and values
        13. Uninsured or Underinsured Motorist Bodily Injury
            - Extract only if field Uninsured or Underinsured Motorist Bodily Injury with labels  (if same label is present just ignore the label , if different label is present then have to extract with label)
        14. Extract only if field Uninsured or Underinsured Motorist Bodily Injury with labels  (if same label is present just ignore the label , if different label is present then have to extract with label)
            - Extract only if field Uninsured Motorist Property Damage with labels  (if same label is present just ignore the label , if different label is present then have to extract with label and values)
        15. Medical Payments
            - Extract only if field Medical Payments  (if same label is present just ignore the label , if different label is present then have to extract with label and value)
        16. Comprehensive
            - Extract only if field Comprehensive  (if same label is present just ignore the label , if different label is present then have to extract with label and value )
        17. Collison
            - Extract only if field Collision  (if same label is present just ignore the label , if different label is present then have to extract with label and value)
        18. Roadside Assistance
            -Extract only if field Roadside Assistance  (if same label is present just ignore the label , if different label is present then have to extract with label and value)
        19. Towing and Labour
            - Extract only if field Towing and Labour  (if same label is present just ignore the label , if different label is present then have to extract with label)
        20. Vehicles Info (extract following datapoints)
                1. VIN
                2. Year
                3. Make
                4. Model
        ...
        21. Driver Info
            - Extract only if field Driver Info  (if same label is present just ignore the label , if different label is present then have to extract with label and value)
        22. Deductibles
            - Extract only if field Deductible or Retensions   (if same label is present just ignore the label , if different label is present then have to extract with label and value)
        23. Terrorism
            - extract all the data  if field as Terrorsim   (if same label is present just ignore the label , if different label is present then have to extract with label and value)
        24. Exclusions
            - extract all the data  if field as Exclusions
...
def prompt_personal_umbrella(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Personal Umbrella insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        10. Miscellaneous Premium (the miscellaneouspremium is mentioned as miscellaneouspremium only there is no other alternatives names)
            - Only if labeled exactly 'miscellaneous premium'.
        11. Umbrells Limits
            - Extract the umbrella limits with labels and values
        12. Underlying Policies
            -Extract following datapoints
            1.Underlying Carrier
            2.Underlying Coverage
        ...
        15. Exclusions
            - extract all the data  if field as Exclusions
        16. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
            -Scan to END of document.
            -Merge multi-line entries into a single entry.
            -Format EXACTLY as:
...
def prompt_inland_marine(large_text=None):
    return f""""
        You are an expert insurance data extractor.
        The following text is extracted from a Inland Marine insurance PDF using a mix of
        digital text, OCR text, and tables.  
        Some forms or endorsements may be spread across multiple pages or split into separate
        lines. Your task is to search the **ENTIRE** document and extract every required
        datapoint listed below.  
        ...
        11. Location
            ...
            - Example:"Loc 1  1731 W Business U.S. 60, ---- if any one those are lisited in the document we have to extract in location                                                                                                   
            - sometimes BLDG label is also present in some document , in that case the output should be like -- "Loc 1 - BLDG 1 - 1731 W Business U.S. 60
        12. Inland Marine Details
            - Extract inland marine coverage information from the Inland Marine section or applicable endorsement.
        13. Equipment Schedule
            - Extract listed equipment details from the Equipment Schedule section.
        14. Deductibles
            - Extract all deductible values from the deductible section.
        15. Loss Payee
            - Extract entities listed as loss payees from the Loss Payee section.
        16. Rental equipment from others
            - Extract details related to rental equipment obtained from other parties.
        17. Rental equipment to others
            - Extract details related to equipment rented out to others.
        18. Installation floater
            - Extract all installation floater coverage information.
        19. Terrorism
            - Extract terrorism coverage details whether listed in premium summary or a separate terrorism page.
        20. Exclusions
            - Extract all exclusions listed under the exclusions section.
        21. Additional Coverage
            - Extract all exclusions listed under the exclusions section.
        22. Forms And Endorsements — extraction rules
            -Extract ONLY from the "Forms And Endorsements" section.
...
def prompt_master_pkg(large_text=None):
    return f"""
        You are an expert insurance data extractor.
        The following text is extracted from a Master BOP insurance PDF using a mix of
        digital text, OCR text, and tables.  
